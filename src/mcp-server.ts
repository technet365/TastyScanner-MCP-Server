...
// ============================================================================
// FIX: Each session gets its own McpServer instance so multiple clients
//      (DeerFlow + n8n) can connect simultaneously.
// ============================================================================
...
// ---------------------------------------------------------------------------
// TastyTrade Client (shared singleton — stateless, safe to share)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Helper functions (shared across all sessions)
// ---------------------------------------------------------------------------
...
// Each session gets its own instance so multiple clients can connect at once.
// The tastyClient singleton is shared (it's just an HTTP client, no state).
// ---------------------------------------------------------------------------
...
async function main() {
  ...
  // --- Session tracking ---
  // Each session has its own McpServer + transport pair.
      ...
      if (sessionId && activeSessions.has(sessionId)) {
        ...
        transport = new StreamableHTTPServerTransport({
          ...
          onsessioninitialized: (sid) => {
            activeSessions.set(sid, { transport, server: sessionServer });
        ...
        // KEY FIX: each session gets its own McpServer instance
